...
class Keyboard
{
  public:
    ...
    {
        return sf::Keyboard::isKeyPressed(ConvertChip8ToQwerty(key));
    };

    static std::optional<uint8_t> WaitForKeyPress(Window& window)
    {
        while (true)
        {
            std::optional<sf::Keyboard::Key> key{ window.WaitKeyPress() };
            if (!key.has_value())
                return std::nullopt;
